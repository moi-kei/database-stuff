...
FROM rider INNER JOIN users ON (rider.user_id = users.id) 
...
FROM food_item A JOIN restaurant ON (restaurant_id =restaurant.id)
WHERE 10 < ALL
(
	SELECT price 
	FROM food_item 
...
--8 NOT WORKS
SELECT first_name, last_name, SUM(CASE WHEN main = 1 THEN 1 ELSE 0 END) AS `main`, SUM(CASE WHEN main = 0 THEN 1 ELSE 0 END) AS `other`
FROM address
JOIN users ON (user_id = users.id)
WHERE `main` > 40
GROUP BY first_name;

SELECT first_name, last_name, SUM(main), COUNT(*) - SUM(main) AS X
FROM orders
INNER JOIN address ON (customer_id = user_id)
INNER JOIN users A ON (user_id = A.id)
WHERE (
	SELECT COUNT(*)
	FROM orders
	INNER JOIN address ON (customer_id = user_id)
	INNER JOIN users B ON (user_id = B.id)
	WHERE A.id = B.id) >= 40 AND main != 0
GROUP BY first_name;
...
--10 GOOD
SELECT CONCAT(ROUND(SUM(CASE WHEN estimated_delivery_time >= actual_delivery_time THEN 1 ELSE 0 END) *100 / COUNT(*),2), "%") "on time" 
FROM orders 
...
WHEN (
	...
	FROM order_item 
	INNER JOIN food_item ON ( food_item_id = food_item.id) 
	WHERE order_id = A.id)  >= 30 THEN '0'
WHEN (
	SELECT SUM(price * quantity) 
	FROM order_item 
	INNER JOIN food_item ON ( food_item_id = food_item.id) 
	WHERE order_id = A.id) < 30 
AND (
	SELECT SUM(price * quantity) 
	FROM order_item 
	INNER JOIN food_item ON ( food_item_id = food_item.id) 
	WHERE order_id = A.id) >= 20 THEN '1'
WHEN (
	SELECT SUM(price * quantity) 
	FROM order_item 
	INNER JOIN food_item ON ( food_item_id = food_item.id) 
	WHERE order_id = A.id) < 20 
AND (
	SELECT SUM(price * quantity) 
	FROM order_item 
	INNER JOIN food_item ON ( food_item_id = food_item.id) 
	WHERE order_id = A.id) >= 10 THEN '2'
ELSE '3'
END "delivery", 
ROUND((
	SELECT SUM(price * quantity) 
...
CASE 
	...
	ELSE '3'
END,2) "total", 
delivery_charge "stored delivery", total_price "stored total"
FROM orders A
...
--12 DONE
SELECT DISTINCT DAYNAME(order_date) AS day,
...
GROUP BY day
UNION
SELECT CASE WHEN DAYNAME(order_date) = '' THEN "All Days" ELSE "All days" END AS Total,
SUM(CASE WHEN TIME(order_date) BETWEEN '11:00:00' AND '11:59:59' THEN 1 ELSE 0 END) "1100",
SUM(CASE WHEN TIME(order_date) BETWEEN '12:00:00' AND '12:59:59' THEN 1 ELSE 0 END) "1200",
SUM(CASE WHEN TIME(order_date) BETWEEN '13:00:00' AND '13:59:59' THEN 1 ELSE 0 END) "1300",
SUM(CASE WHEN TIME(order_date) BETWEEN '14:00:00' AND '14:59:59' THEN 1 ELSE 0 END) "1400",
...
--13 GOOD
SELECT post_code, first_name, last_name, count(orders.id) "orders", 
...
--14 FUCK IT
SELECT A.id AS id1, B.id AS id2, A.first_name AS first_name, A.last_name AS last_name
FROM users A
INNER JOIN users B ON (A.first_name = B.first_name AND A.last_name = B.last_name AND A.id != B.id)
GROUP BY A.first_name, A.last_name
ORDER BY A.id;

SELECT id1, id2, first_name, last_name
FROM food_item 
JOIN order_item ON (food_item.id = food_item_id) 
...
JOIN (
	...
	INNER JOIN users B ON (A.first_name = B.first_name AND A.last_name = B.last_name AND A.id != B.id)
	GROUP BY A.first_name, A.last_name
	ORDER BY A.id) ON (id1 = customer_id)
WHERE vegetarian = 1;
GROUP BY id1;
...
FROM
(
	SELECT DISTINCT customer_id, COUNT(DISTINCT orders.id), SUM(vegetarian)
	FROM food_item
	INNER JOIN order_item ON (food_item.id = food_item_id)
	INNER JOIN orders ON (order_id = orders.id)
	GROUP BY customer_id
	HAVING SUM(vegetarian) = 0;
	LIMIT 100;
)
HAVING SUM(CASE WHEN vegetarian = 1 THEN 1 ELSE 0) = SUM(vegetarian);

SELECT DISTINCT customer_id, SUM(vegetarian) AS totalveg, COUNT(DISTINCT orders.id) AS totalorders
FROM food_item
INNER JOIN order_item ON (food_item.id = food_item_id)
INNER JOIN orders ON (order_id = orders.id)
GROUP BY customer_id
...
SELECT COUNT(DISTINCT orders.id), SUM(vegetarian)
FROM food_item
INNER JOIN order_item ON (food_item.id = food_item_id)
INNER JOIN orders ON (order_id = orders.id)
WHERE customer_id = 25
...
SELECT ROUND(SUM(
	CASE 
	WHEN SIGN(TIMEDIFF(TIMEDIFF(actual_delivery_time, order_date), "00:40:00")) = -1
	THEN 1 
	ELSE 0 
	END
) / COUNT(*),2) "P" 
